...
Где `sys_id=procedure` определяет термин, значение которого мы хотим узнать (по сути, ответ на вопрос "Что такое "технология"? ). Это позволяет относительно легко добавить поддержку базы знаний для текущих проектов АСУТП. Таким образом, реализована интерактивная интеллектуальна справочная система для проектов АСУТП. Ниже приведены иллюстрации запроса информации из проект и результат - веб-страница с поясняющей информацией.

>Предлагается вставить картинку из проекта Хуторки и ответ на запрос об операциях приготовления творожной массы.
