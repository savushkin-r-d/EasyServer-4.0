...
**Аннотация.**  В данной работе предлагается продолжение применения онтологического подхода к проектированию предприятий рецептурного производства. В рамках продолжения формализации стандартов рассматривается графическое представления внешнего языка спецификации процедурных моделей Procedure Function Chart и P&ID схем (стандарт ISA-88). В результате инженер получает привычный инструмент для работы. Реализованы агенты получения дополнительной информации и анализа типовой нештатной ситуации. Рассматривается также агентно-ориентированный подход к организации взаимодействия роботов в рамках роботизированных производственных комплексов, основанный на взаимодействии через общую семантическую память.
...
* база знаний системы организована иерархически, т.е. представляет собой иерархию предметных областей и соответствующих им онтологий.
...
Разработка комплексной системы информационного обслуживания предполагает выделение нескольких подсистем, ориентированных на решение задач определенных классов. В настоящее время практически на любом предприятии некоторые из этих подсистем уже реализованы с использованием современных технологий и выполняют свои классические функции.

К основным производственным подсистемам относятся (от более низкого уровня к более высокому):
1. SCADA - решается задача диспетчерского управления технологическим процессом (оператором), сбора и архивации технологических данных;
2. MES и WMS
  * MES (управление производственными процессами) - осуществляется выпуск продукции в рамках производства.
  * WMS (управление складом) - осуществляется хранение готовой продукции до отгрузки заказчику.
3. ERP (планирование ресурсов предприятия) - определяет, какой продукт в каких количествах и на каких производственных мощностях будет производится.

При этом каждый уровень связан не только с последующим, но и с предыдущим. Таким образом, одной из задач при построении и внедрении системы информационного обслуживания является интеграция уже существующих подсистем в единую систему и обеспечение возможности развития указанных систем без разрушения единой комплексной системы информационного обслуживания. В конечном итоге построение такой системы предполагает переход от интеграции разнородных подсистем к единому унифицированному технологическому фундаменту для реализации всех подсистем перечисленных классов.

В рамках подхода предполагается рассматривать систему информационного обслуживания как ostis-систему (систему, построенную по Технологии OSTIS). Классическая архитектура ostis-системы [Голенков2018] включает платформу интерпретации семантических моделей ostis-систем, которая в общем случае может быть реализована по-разному, и не зависящую от платформы семантическую модель данной ostis-системы (sc-модель ostis-системы), которая, в свою очередь, включает следующие компоненты:
* абстрактная семантическая графодинамическая память;
* семантическая модель базы знаний, в основе которой лежит иерархическая система предметных областей и соответствующих им онтологий [давыденко2018];
* семантическая модель решателя задач, в рамках которой решатель трактуется как иерархическая система агентов, управляемых событиями в общей семантической памяти и взаимодействующих посредством этой же памяти [шункевич2018];
* семантическая модель интерфейса ostis-системы (включая пользовательский интерфейс), который трактуется как подсистема определенного рода, имеющая свою базу знаний и свой решатель задач.

В работе [Савушкин2017] были рассмотрены верхние уровни иерархической системы предметных областей и соответствующих им онтологий, связанных с формализацией стандарта ISA-88. В рамках дальнейшей работы потребовалось расширение указанной системы предметных областей, были построены фрагменты онтологий для следующих предметных областей (*сделать SCn*):
* Предметная область веществ
  * Предметная область продуктов производства
* Предметная область персонала предприятия
* Предметная область производственных ситуаций
  * Предметная область нештатных производственных ситуаций
* Предметная область логистических процессов
  * Предметная область процессов на складе готовой продукции
  * Предметная область процессов доставки готовой продукции

Решатель задач систему информационного обслуживания в настоящее время включает набор базовых поисковых агентов, а также ряд агентов, осуществляющих решение более сложных задач, например, выявление причин каких-либо ситуаций. Пример работы такого агента рассмотрен ниже в разделе 3.1.

Интерфейс системы информационного обслуживания можно условно разделить на две части:
...
Ниже приведены примеры работы текущей реализации интерфейса системы как с пользователем, так и с другими системами, в частности, SCADA-системой.
...
Например, если задержка в процессе приготовления партии превышает определенную величину, то необходимо отложить время загрузки данной партии для доставки заказчику, в противном случае машина приедет и будет ожидать, пока продукт охладится на складе. Система информационного обслуживания  может отслеживать такие ситуации и, в случае необходимости, изменять время выезда машины на склад для погрузки с учетом нового времени готовности партии.

Пример правила в базе знаний, описывающего такого рода корректировку, представлен на рисунке Х (предполагается, что задержка не должна превышать 30 мин).

![logistics](images/logistics.png "Рисунок - Правило, описывающее корректировку в случае задержки на производстве")

Рисунок - Правило, описывающее корректировку в случае задержки на производстве

### 3.3 Реализация робототехнической подсистемы

Важнейшим компонентом системы информационного обслуживания является подсистема, ориентированная на работу с роботизированными установками. Согласно изложенным выше принципам построения системы автоматизации предприятия, все участники процесса производства, включая роботов, трактуются как агенты, работающие над общим информационным пространством. Таким образом, с одной стороны, роботы фиксируют результаты своей деятельности в общей базе знаний, с другой - могут управляться ситуациями и событиями в базе знаний. Рассмотрим более подробно использование робототехнической подсистемы на примере линии производства творога.

Линия производства творога на данный момент включает в себя два роботизированных узла: узел упаковки продукта в короба с помощью коллаборативного робота и узел транспортировки паллет с готовым продуктом в приемный порт склада на основе мобильного робота.

Рассмотрим задачу упаковки творогов в короба коллаборативным роботом. Общий вид установки приведен на рисунке Х.

![khutor_robot](images/khutor_robot.png "Рисунок - Установка по укладке творогов")

Рисунок - Установка по укладке творогов

Установка получает на вход поток творогов, который затем разделяется и формируется в группы по 12 шт. Робот с помощью захватного механизма забирает партию и укладывает в две коробки. Упаковка творога допускается в два типа коробок: одноуровневые по 6 шт. и в двухуровневые по 12. Далее коробки по выходному транспортеру выдаются на узел паллетизации. Для работы системы роботу необходима следующая информация, получаемая от системы информационного обслуживания:
...
На основе данной информации программное обеспечение робота гибко реагирует на текущие параметры всего производственного процесса. Подстраивая скорость и задержки в выполнении операций робот динамически управляет производительностью ячейки, чтобы не создавать очереди на входном и выходном конвейере. Тип подаваемой коробки определяет количество продукта, которое роботу необходимо уложить в нее. Состояние соседних узлов производственного процесса позволяет роботу переключить процесс упаковки в режим ручной обработки или пустить продукт во временный накопитель.

Далее рассмотрим задачу транспортировки готового продукта на склад с помощью мобильного робота. Вид робота приведен на рисунке Х.

![khutor_mobile](images/khutor_mobile.png "Рисунок - Мобильный транспортный узел")

Рисунок - Мобильный транспортный узел
...
Конструирование миссий для робота выполняется системой информационного обслуживания, для этого решаются следующие подзадачи:
...
В работе рассмотрены принципы построения и текущая версия системы информационного обслуживания предприятия рецептурного производства на примере ОАО "Савушкин продукт".

В планах развития рассмотренной системы можно выделить следующие актуальные задачи:

1. Отказ от редактирования исходных текстов базы знаний в пользу редактирования базы знаний в реальном времени, что сделает более эффективным работу пользователей и позволит сотрудникам предприятия (например, инженеру по автоматизации) вносить изменения в базу знаний самостоятельно.
2. Доработка графического представления: в инженерной документации имеет значение каждая мелочь (толщина, цвет и изломы линии), в текущей реализации отображение несколько упрощено.
3. Более тесная интеграция уже реализованных систем с системой информационного обслуживания, построение единой системы, объединяющей в себе несколько уровней предприятия.
