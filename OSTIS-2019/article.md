...
Пример того, как решается какая-то задача - просмотр информации об управляющем рецепте. Оператор (мастер) хочет получить дополнительную справочную информацию о текущем управляющем рецепте. Для этого он нажимает на соответствующую кнопку в проекте SCADA-системы "Хуторок":
...
Пример того, как решается комплексная задача логистического взаимодействия производства, склада и транспорта - контролирование выполнения производственной партии продукции. Для минимизации расходов, необходимо обеспечить минимальную задержку между приготовлением партии продукта, охлаждением его на складе, погрузкой и доставкой транспортом к заказчику. Любое ожидание и простои (ожидание приготовления, охлаждение продукта и простои машины во время ожидания загрузки) увеличивают себестоимость продукции. Поэтому все этапы нужно четко контролировать и при возникновении задержек оперативно изменять времена начала следующих этапов. Например, приготовление партии заключается в выполнении одного или нескольких рецептов на производственных аппаратах (S88), рецепты состоят из последовательно выполняемых операций. Если операция во время выполнения становится на паузу - это означает, что время приготовления рецепта увеличивается - сведения о данном событии могут быть переданы в базу знаний. Если задержка превышает определенную величину - значит необходимо также отложить время загрузки данной партии для доставки заказчику, иначе машина приедет и будет ожидать пока продукт охладиться на складе. База знаний может отслеживает такие ситуации и в случае необходимость изменять время выезда машины на склад для погрузки с учетом нового времени готовности партии.
...
<!-- NeedHelp Здесь я планировал описать программу в семантической памяти, которую мог бы теоретически выполнить робот, или наоборот, какие-либо данные получены с робота, на основе которых мы могли бы сделать какие-то выводы. Нужен детальный пример -->

### 3.4 Реализация пользовательского интерфейса системы

Рассказать про подход семантика-синтаксис-визуализация.
Пример отображения PFC
Сделать скрины
Пример отображения P&ID
Сделать скрины
